...
namespace SO
{
    ...
    public class EventManagerWindow : EditorWindow
    {
        ...
        private Dictionary<EventSO, List<GameObject>> gameEventDictionary = new Dictionary<EventSO, List<GameObject>>();
        ...
        private EventListenerSO[] eventObjects;
        ...
        [MenuItem("Window/SOEventManager")]
        ...
        private void DrawActiveSceneEvents()
        {
            ...
            foreach (EventSO key in gameEventDictionary.Keys)
            {
                //***********************Event Name***********************
                EditorGUILayout.BeginHorizontal(EditorStyles.helpBox); //All the Event names
        ...
        private void GetAllScriptableEventsInScene()
        {
            ...
            eventObjects = FindObjectsOfType<EventListenerSO>();

            foreach (EventListenerSO listener in eventObjects)
            {
                foreach (SOListener _listener in listener.listeners)
                {

                    if (_listener.Event != null) //Check to make sure there is an event (there won't be when the component is first added)
                    {
                        //Check to see if the key does not already exist in the dictionary
                        if (!gameEventDictionary.ContainsKey(_listener.Event))
                        {
                            List<GameObject> eventObjectList = new List<GameObject>();

                            eventObjectList.Add(listener.gameObject);

                            gameEventDictionary.Add(_listener.Event, eventObjectList); //Add the event and the gameObject list

                        }
                        else
                        {
                            List<GameObject> copyList = gameEventDictionary[_listener.Event]; //Get a copy of the list associated with listener event
                            copyList.Add(listener.gameObject);

                            gameEventDictionary[_listener.Event] = copyList;
                        }
                    }
